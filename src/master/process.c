...
process_add(char *name, char *path, char *arglist, unsigned int flags)
{
    ...
    dax_log(DAX_LOG_DEBUG,"Adding process %s to configuration",name);
    ...
    while(this != NULL) {
        ...
        if(this->delay > 0) {
            ...
            while(result) {
                ...
                if(errno == EFAULT)
                    dax_log(DAX_LOG_FATAL, "Serious problem with nanosleep in function process_start_all()");
                assert(errno != EINVAL);
            }
        }
        this = this->next;
...
_set_user_group(dax_process *proc)
{
    ...
    if(proc->uid >= 0 && proc->uid != uid) {
        dax_log(DAX_LOG_MINOR, "Changing user id to %d process %s", proc->uid, proc->name);
        result = setuid(proc->uid);
        if(result) {
            dax_log(DAX_LOG_ERROR, "Unable to set uid %d - %s", proc->uid, strerror(errno));
        }
    }
    if(proc->gid >= 0 && proc->gid != gid) {
        dax_log(DAX_LOG_MINOR, "Changing group id to %d process %s", proc->gid, proc->name);
        result = setgid(proc->gid);
        if(result) {
            dax_log(DAX_LOG_ERROR, "Unable to set gid %d - %s", proc->gid, strerror(errno));
        }
    }
}
...
process_start(dax_process *proc)
{
    ...
    if(proc) { /* We are the parent */
        if(result) dax_log(DAX_LOG_ERROR,"Unable to properly set up pipes for %s\n", proc->name);
        child_pid = fork();
        if(child_pid > 0) { /* This is the parent */
            proc->pid = child_pid;
            proc->exit_status = 0;

            dax_log(DAX_LOG_MAJOR, "Starting Process - %s - %d",proc->path,child_pid);
...
_cleanup_process(pid_t pid, int status)
{
    ...
    if(proc) {
        dax_log(DAX_LOG_MINOR, "Cleaning up Process %d - Returned Status %d", pid, status);
        proc->pid = 0;
        proc->last_etime = 0;
        proc->last_ptime = 0;
        ...
        dax_log(DAX_LOG_ERROR, "Process %d not found for cleanup", pid);
        return ERR_NOTFOUND;
    }
    return 0;
}
...
_process_get_cpu_stat(dax_process *proc)
{
    ...
    f = fopen(filename, "r");
    if(f == NULL) {
        dax_log(DAX_LOG_ERROR, "Unable to open file %s", filename);
        return errno;
    }
    ...
    f = fopen("/proc/stat", "r");
    if(f == NULL) {
        dax_log(DAX_LOG_ERROR, "Unable to open file proc/stat for pid %d", proc->pid);
        return errno;
    }
...
{
    dax_log(DAX_LOG_MAJOR, "Restarting Process - %s - Delay = %d mSec", proc->name, proc->restartdelay);
    proc->restartcount++;
    process_start(proc);
        ...
        if((this->flags & PFLAG_RESTART) && this->state == PSTATE_DEAD ) {
            ...
            if((count + this->pid) % 64 == 0) {
                ...
                if(this->state == PSTATE_CONDEMNED) {
                    dax_log(DAX_LOG_MODULE, "Killing process %s [%d]", this->name, this->pid);
                    kill(this->pid, SIGKILL);
                }
                _process_get_cpu_stat(this);
                if(this->cpu > 0.0 && this->pcpu > this->cpu) {
                    dax_log(DAX_LOG_MODULE, "Process %s [%d] has exceeded CPU usage of %f",
                            this->name, this->pid, this->cpu);
                    this->state = PSTATE_CONDEMNED;
                }
                if(this->mem > 0 && this->rss > this->mem) {
                    dax_log(DAX_LOG_MODULE, "Process %s [%d] has exceeded Memory usage of %ld",
                            this->name, this->pid, this->mem);
                    this->state = PSTATE_CONDEMNED;
                }
                /* The first time through we terminate the process the easy way */
                if(this->state == PSTATE_CONDEMNED) {
                    dax_log(DAX_LOG_MODULE, "Commanding process %s [%d] to quit",
                            this->name, this->pid);
                    kill(this->pid, SIGQUIT);
                }
                dax_log(DAX_LOG_MODULE, "%s: PID = %d, CPU = %f%%, Memory = %ld kb", this->name, this->pid, this->pcpu, this->rss);
            }
        }
        this = this->next;
    }
