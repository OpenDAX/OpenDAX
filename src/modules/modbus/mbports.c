...
openIPport(mb_port *mp, struct in_addr address, uint16_t port)
{
    ...
    if(result) {
        dax_log(ds, LOG_ERROR, "Unable to set socket to non blocking");
        return -1 ;
    }
    return fd;
}
...
mb_new_port(const char *name, unsigned int flags)
{
    ...
    if(mport != NULL) {
        ...
        if(mport->connections == NULL) {
            dax_log(ds, LOG_ERROR, "Unable to allocate initial connection pool");
            return NULL;
        }
    }
    return mport;
...
mb_set_serial_port(mb_port *port, const char *device, int baudrate, short databits, short parity, short stopbits)
{
    ...
    if(port->device == NULL) {
        dax_log(ds, LOG_ERROR, "%s - Unable to allocate space for port", __func__);
        return MB_ERR_ALLOC;
    }

    port->baudrate = getbaudrate(baudrate);
    if(port->baudrate == 0) {
        dax_log(ds, LOG_ERROR, "Bad baudrate passed");
        return MB_ERR_BAUDRATE;
    }
    if(databits >= 5 && databits <= 8) {
        port->databits = databits;
    } else {
        dax_log(ds, LOG_ERROR, "Wrong number of databits passed");
        return MB_ERR_DATABITS;
    }
    if(stopbits == 1 || stopbits == 2) {
        port->stopbits = stopbits;
    } else {
        dax_log(ds, LOG_ERROR, "Wrong number of stopbits passed");
...
mb_set_network_port(mb_port *port, const char *ipaddress, unsigned int bindport, unsigned char socket)
{
    ...
    if(socket == UDP_SOCK || socket == TCP_SOCK) {
        ...
        dax_log(ds, LOG_ERROR, "Bad argument for socket");
...
mb_close_port(mb_port *port)
{
    ...
    if(port->devtype == MB_NETWORK) {
        for(int n=0; n<port->connection_count; n++) {
            ...
            if(result) {
                dax_log(ds, LOG_ERROR, "Error closing network file descriptor %d", port->connections[n].fd);
            }
        }
        port->connection_count = 0;
         ...
         port->fd = 0;
         if(result) {
              dax_log(ds, LOG_ERROR, "Error closing file descriptor %d", port->fd);
         }
    }
    return 0;
}
