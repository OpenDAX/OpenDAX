...
_mb_read(mb_port *port, int fd)
{
    ...
    while(1) {
        if(state == 0) { /* Waiting */
            ...
            if(result < 0) {
                dax_log(DAX_LOG_ERROR, "Error reading Serial port on fd = %d", fd);
    ...
    if(buffindex > 0) {
        ...
        if(result) {
            ...
            if(result > 0) { /* We have a response */
                ...
                dax_log(DAX_LOG_ERROR, "Error reading serial port data %d\n", result);
                return result;
            }
        }
    }
...
slave_loop(mb_port *port) {
   ...
   while(1) {
        ...
        if(result) {
            dax_log(DAX_LOG_ERROR, "Exiting slave loop because of error %d\n",  result);
            return result;
        }
    }
}
