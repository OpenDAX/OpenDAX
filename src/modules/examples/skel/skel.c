...
int main(int argc,char *argv[]) {
    ...
    if(result) {
        dax_log(ds, LOG_FATAL, "Unable to create tag - %s", tagname);
    }
    ...
    if(str == NULL) {
        dax_log(ds, LOG_FATAL, "Unable to allocate memory");
    }
    strcpy(str, tagname);
    str = strcat(str, "[2]");
    ...
    if(result) {
        dax_log(ds, LOG_FATAL, "Unable to retrive Handle for tag - %s", tagname);
    }

    dax_mod_set(ds, MOD_CMD_RUNNING, NULL);
    dax_log(ds, LOG_MINOR, "Skeleton Module Starting");
    while(1) {
