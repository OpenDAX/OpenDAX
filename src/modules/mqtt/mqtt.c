...
{
    dax_log(ds, LOG_MINOR, "Connection lost - %s", cause);
    _connected = 0;
}
...
subscribe(subscriber_t *sub) {
    int result;
    if(sub->tag_count == 0) {
        dax_log(ds, LOG_ERROR, "No tags given for topic %s", sub->topic);
    ...
    for(int n=0;n<sub->tag_count;n++) {
        ...
        if(result) {
            dax_log(ds, LOG_ERROR, "Unable to add tag %s as subscription", sub->tagnames[n]);
            return 1;
        }
    }
...
publish(publisher_t *pub) {
    ...
    if(pub->tag_count == 0) {
        dax_log(ds, LOG_ERROR, "No tags given for topic %s", pub->topic);
    ...
    for(int n=0;n<pub->tag_count;n++) {
        ...
        if(result) {
            dax_log(ds, LOG_ERROR, "Unable to add tag %s to publication", pub->tagnames[n]);
            return 1;
        }            
    }
    if(pub->update_tag == NULL) {
        for(int n=0;n<pub->tag_count;n++) {
            ...
            if(result) {
                dax_log(ds, LOG_ERROR, "Problem adding update event for tag %s", pub->tagnames[n]);
                return 1;
            }
        }
    }
...
main(int argc,char *argv[]) {
    ...
    ds = dax_init("daxmqtt");
    if(ds == NULL) {
        dax_log(ds, LOG_FATAL, "Unable to Allocate Dax State Object\n");
        exit(-1);
    }
    ...
    dax_log(ds, LOG_MINOR, "MQTT Module Starting");
    client_loop();
