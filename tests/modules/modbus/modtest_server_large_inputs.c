...
main(int argc, char *argv[])
{
    ...
    dax_state *ds;
    tag_handle h;
    u_int16_t buff[1024], rbuff[1024];
    ...
    bzero(buff, h.size);
    buff[0]=0xA589;
    result =  dax_tag_handle(ds, &h, "mb_ireg[3000]", 1);
    if(result) return result;
    dax_write_tag(ds, h, buff); /* Write it to OpenDAX */
    exit_status += read_input_registers(s, 3000, 1, rbuff);
    printf("rbuff[0] = 0x%X\n", rbuff[0]);
    if(rbuff[0] != buff[0]) exit_status++;

    for(i=0;i<64;i++) buff[i] = 1024 + i;
    result =  dax_tag_handle(ds, &h, "mb_ireg[3936]", 64);
    if(result) return result;
    dax_write_tag(ds, h, buff); /* Write it to OpenDAX */
    exit_status += read_input_registers(s, 3936, 64, rbuff);
