...
/*
 *  This test writes a tag that is larger than a single message
 */
...
do_test(int argc, char *argv[])
{
    ...
    for(int n=0;n<4096;n++) {
        temp_array[n] = n;
        if(n%2 == 0) {
            temp_mask[n] = 0xFFFFFFFF;
        } else {
            temp_mask[n] = 0x00000000;
        }

    }
    dax_tag_mask(ds, h, temp_array, temp_mask);
    ...
    for(int n=0;n<4096;n+=512) {
        ...
        dax_tag_read(ds, h1, &temp_array[n]);
    }
