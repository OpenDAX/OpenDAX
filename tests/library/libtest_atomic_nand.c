...
_bool_test(dax_state *ds) {
    ...
    if(dax_atomic_op(ds, h2, temp1, ATOMIC_OP_NAND)) return -1;
    if(dax_read_tag(ds, h, output)) return -1;
    DF("in     = %02X %02X", temp1[0], temp1[1]);
    DF("output = %02X %02X", output[0], output[1]);
    if(output[0] != 0xAD || output[1] != 0x5A) return -1;
    ...
    if(output[0] != 0xAB || output[1] != 0xFE || output[2] != 0x55) return -1;
...
_byte_test(dax_state *ds) {
    ...
    if(dax_atomic_op(ds, h, input2, ATOMIC_OP_NAND)) return -1;
    if(dax_read_tag(ds, h, output)) return -1;
    for(int n=0;n<4;n++) {
        if(output[n] != (dax_byte)~(input1[n] & input2[n])) {
            DF("in1 = 0x%02X", input1[n]);
            DF("in1 = 0x%02X", input2[n]);
            DF("out = 0x%02X", output[n]);
            return -1;
        }
    }
    return 0;
...
_real_test(dax_state *ds) {
    ...
    if(result) return -1;
    result = dax_atomic_op(ds, h, temp2, ATOMIC_OP_NAND);
