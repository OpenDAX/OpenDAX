...
_bool_test(dax_state *ds) {
    ...
    if(dax_atomic_op(ds, h, temp1, ATOMIC_OP_XOR)) return -1;
    if(dax_read_tag(ds, h, &output)) return -1;
    if(output[0] != 0x5F || output[1] != 0xFA) {
        return -1;
    }
    ...
    if(dax_atomic_op(ds, h2, temp1, ATOMIC_OP_XOR)) return -1;
    if(dax_read_tag(ds, h, &output)) return -1;
    if(output[0] != 0xFD || output[1] != 0x5A) return -1;
    ...
    if(dax_atomic_op(ds, h2, temp1, ATOMIC_OP_XOR)) return -1;
    if(dax_read_tag(ds, h, &output)) return -1;
    if(output[0] != 0xB5 || output[1] != 0xB4 || output[2] != 0x54) return -1;
...
_byte_test(dax_state *ds) {
    ...
    if(dax_atomic_op(ds, h, input2, ATOMIC_OP_XOR)) return -1;
    if(dax_read_tag(ds, h, output)) return -1;
    for(int n=0;n<4;n++) {
...
_real_test(dax_state *ds) {
    ...
    if(result) return -1;
    result = dax_atomic_op(ds, h, temp2, ATOMIC_OP_XOR);
