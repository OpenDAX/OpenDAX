...
__add_tags(void)
{
	...
	if(result) {
        dax_log(ds, LOG_FATAL, "Can't Add Tags");
        kill(getpid(), SIGQUIT);
    }
...
main(int argc,char *argv[])
{
    ...
    dax_log(ds, LOG_MINOR, "Starting bad module");
    ...
    if(quit_time > 0) {
        usleep(quit_time);
        dax_log(ds, LOG_FATAL, "Quiting because I'm out of time");
    }
    ...
    while(1) {
        if(_caught_signal == SIGQUIT) {
            ...
            if(!no_quit) {
                dax_log(ds, LOG_MAJOR, "Gotta Go");
                exit(0);
            } else {
                dax_log(ds, LOG_MAJOR, "I don't think so scooter");
            }
            _caught_signal = 0;
        }
