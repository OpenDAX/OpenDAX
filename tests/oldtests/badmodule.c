...
__add_tags(void)
{
	...
	if(result) {
        dax_log(DAX_LOG_FATAL, "Can't Add Tags");
        kill(getpid(), SIGQUIT);
    }
...
main(int argc,char *argv[])
{
    ...
    dax_log(DAX_LOG_MINOR, "Starting bad module");
    ...
    if(dax_connect(ds))
        dax_log(DAX_LOG_FATAL, "Unable to register with the server");
    __add_tags();
    ...
    if(quit_time > 0) {
        usleep(quit_time);
        dax_log(DAX_LOG_FATAL, "Quiting because I'm out of time");
    }
    ...
    while(1) {
        if(_caught_signal == SIGQUIT) {
            ...
            if(!no_quit) {
                dax_log(DAX_LOG_MAJOR, "Gotta Go");
                exit(0);
            } else {
                dax_log(DAX_LOG_MAJOR, "I don't think so scooter");
            }
            _caught_signal = 0;
        }
