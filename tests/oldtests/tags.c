...
add_random_tags(int tagcount, char *name)
{
    ...
    for(n = 0; n < tagcount; n++) {
        ...
        if(result < 0) {
            dax_log(DAX_LOG_MINOR, "Failed to add Tag %s %s[%d]", tagname, dax_type_to_string(ds, data_type), count );
            return result;
//        } else {
...
void
send_tag_to_lua(lua_State *L, tag_handle h, void *data)
{
    cdt_iter tag;
 ...
 * cdt iterator. */
int
get_tag_from_lua(lua_State *L, tag_handle h, void* data, void *mask){
    cdt_iter tag;
    struct iter_udata udata;
    int n, offset;
